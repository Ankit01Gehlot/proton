...
void AggregatedDataVariants::serialize(WriteBuffer & wb, VersionType version, const Aggregator & aggregator) const
{
    return aggregator.checkpoint(*this, wb, version);
}

void AggregatedDataVariants::deserialize(ReadBuffer & rb, VersionType version, const Aggregator & aggregator)
{
    return aggregator.recover(*this, rb);
}
            ...
            for (AggregateFunctionInstruction * inst = aggregate_instructions; inst->that; ++inst)
            {
                inst->batch_that->addBatchLookupTable8(
                    ...
                    [&](AggregateDataPtr & aggregate_data)
                    {
                        ...
                        createAggregateStates<use_compiled_functions, /*skip_expanded_data*/ true>(aggregate_data);
                    },
                    state.getKeyData(),
    ...
    KeyHandler && key_handler) const
{
    ...
    auto func = [&](AggregateDataPtr & __restrict dst, AggregateDataPtr & __restrict src, bool inserted)
    {
        if (inserted)
        {
            ...
            createAggregateStates<use_compiled_functions>(dst);
        }
...
/// [Legacy]
template <typename Method>
bool Aggregator::executeAndRetractImplLegacy(
    Method & method,
    Arena * aggregates_pool,
    ...
    for (size_t i = row_begin; i < row_end; ++i)
    {
        ...
        else
        {
            ...
            if (retracted_result.isInserted())
            {
                ...
                createAggregateStates</*use_compiled_functions*/ false, /*skip_expanded_data*/ true>(retracted_data);
    ...
    ManyAggregatedDataVariants & aggregated_data, ManyAggregatedDataVariants & retracted_data) const
{
    ...
    for (size_t result_num = 1, size = retracted_data.size(); result_num < size; ++result_num)
    {
        ...
        src_retracted_table.mergeToViaEmplace(dst_retracted_table, [&](AggregateDataPtr & __restrict dst, AggregateDataPtr & __restrict src, bool inserted) {
            ...
            if (src)
            {
                if (!dst)
                {
                    dst = retracted_res->aggregates_pool->alignedAlloc(total_size_of_aggregate_states, align_aggregate_states);
                    createAggregateStates</*use_compiled_functions*/ false, /*skip_expanded_data*/ true>(dst);
                }
        ...
        dst_retracted_table.forEachValue([&](const auto & key, auto & mapped) {
            ...
            if (!src_retracted_table.find(key))
            {
                ...
                if (find_it)
                {
                    if (!mapped)
                    {
                        mapped = retracted_res->aggregates_pool->alignedAlloc(total_size_of_aggregate_states, align_aggregate_states);
                        createAggregateStates</*use_compiled_functions*/ false, /*skip_expanded_data*/ true>(mapped);
                    }
    ...
    for (size_t result_num = 1, size = aggregated_data.size(); result_num < size; ++result_num)
    {
        ...
        dst_retracted_table.forEachValue([&](const auto & key, auto & mapped) {
            ...
            auto & dst_mapped = dst_it->getMapped();
            if (inserted)
            {
                dst_mapped = res->aggregates_pool->alignedAlloc(total_size_of_aggregate_states, align_aggregate_states);
                createAggregateStates</*use_compiled_functions*/ false, /*skip_expanded_data*/ true>(dst_mapped);
            }
    ...
    auto state_deserializer = [this](auto & place, auto & rb_, Arena * arena) {
        ...
        if (expanded_data_type == ExpandedDataType::UpdatedWithRetracted)
        {
            ...
            if (has_retracted)
            {
                ...
                createAggregateStates</*use_compiled_functions*/ false, /*skip_expanded_data*/ true>(retracted);
                for (size_t i = 0; i < params.aggregates_size; ++i)
    ...
    for (size_t i = row_begin; i < row_end; ++i)
    {
        ...
        else
        {
            ...
            if (!UpdatedDataEx::isEmpty(aggregate_data) && !RetractedDataEx::hasRetracted(aggregate_data))
            {
                ...
                retracted = retracted_pool->alignedAlloc(total_size_of_aggregate_states, align_aggregate_states);
                createAggregateStates</*use_compiled_functions*/ false, /*skip_expanded_data*/ true>(retracted);
    ...
    bool & no_more_keys) const
{
    ...
    if (result.type == AggregatedDataVariants::Type::without_key)
    {
        ...
        if (!UpdatedDataEx::isEmpty(result.without_key) && !RetractedDataEx::hasRetracted(result.without_key))
        {
            ...
            retracted = result.retracted_pool->alignedAlloc(total_size_of_aggregate_states, align_aggregate_states);
            createAggregateStates</*use_compiled_functions*/ false, /*skip_expanded_data*/ true>(retracted);
...
void NO_INLINE Aggregator::mergeUpdatedGroupsImpl(ManyAggregatedDataVariants & non_empty_data, Arena * arena) const
{
    ...
    for (size_t result_num = 1, size = non_empty_data.size(); result_num < size; ++result_num)
    {
        ...
        auto merge_updated_func = [&](const auto & key, auto & mapped) {
            ...
            if (inserted)
            {
                dst_it->getMapped() = arena->alignedAlloc(total_size_of_aggregate_states, align_aggregate_states);
                createAggregateStates</*use_compiled_functions*/ false, /*skip_expanded_data*/ true>(dst_it->getMapped());
            }
        };
    ...
    for (size_t result_num = 1, size = non_empty_data.size(); result_num < size; ++result_num)
    {
        ...
        dst_table.forEachValue([&](const auto & key, auto & mapped) {
            ...
            {
                mapped = arena->alignedAlloc(total_size_of_aggregate_states, align_aggregate_states);
                createAggregateStates</*use_compiled_functions*/ false, /*skip_expanded_data*/ true>(mapped);
