...
void mergeEmitQuerySettings(const ASTPtr & emit_query, WatermarkStamperParams & params)
{
    ...
    if (emit->on_update)
        params.mode = WatermarkEmitMode::OnUpdate;

    if (emit->periodic_interval)
    {
        if (params.mode != WatermarkEmitMode::Unknown)
            throw Exception(ErrorCodes::INCORRECT_QUERY,
                "The 'PERIODIC {}' in emit clause conflicts with watermark emit mode '{}'",
                emit->periodic_interval->formatForErrorMessage(),
    ...
    else
    {
        ...
        else
        {
            ...
            if (mode == WatermarkEmitMode::Unknown)
            {
                ...
                mode != WatermarkEmitMode::Periodic && mode != WatermarkEmitMode::OnUpdate)
                throw Exception(
                    ErrorCodes::UNSUPPORTED_WATERMARK_STRATEGY,
